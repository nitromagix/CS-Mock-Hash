...
checkPassword = async (username, plaintextPassword) => {
  ...
  if (globalStore[username]) {
    ...
    // let result = bcrypt.compare(pwHash, plaintextPassword);
    // Load hash from your password DB.
    result = await bcrypt.compare(plaintextPassword, pwHash);
...
hashPassword = async (username, password) => {
  ...
  const pwHash = await bcrypt.hashSync(pw, rounds);
  ...
  console.log(`username: ${username}, password hash: ${globalStore[username]}`);
};
